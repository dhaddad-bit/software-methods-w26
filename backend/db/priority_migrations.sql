...
ALTER TABLE user_busy_block
  ADD COLUMN IF NOT EXISTS client_request_id TEXT;

CREATE UNIQUE INDEX IF NOT EXISTS uniq_user_busy_block_client_request
  ON user_busy_block (user_id, client_request_id)
  WHERE client_request_id IS NOT NULL;

ALTER TABLE user_busy_block DROP CONSTRAINT IF EXISTS user_busy_block_client_request_id_chk;
ALTER TABLE user_busy_block
  ADD CONSTRAINT user_busy_block_client_request_id_chk
  CHECK (client_request_id IS NULL OR (client_request_id <> '' AND length(client_request_id) <= 128));
